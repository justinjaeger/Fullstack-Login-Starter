 ...
 * - deletes the current access_token if there is one
...
/*
  NOTE: this should be inserted as middleware for 
  LITERALLY EVERY TIME you are doing something
  that requires authorization aka being logged in
 */
...
tokenController.verifyToken = (req, res, next) => {
  ...
  jwt.verify(access_token, ACCESS_TOKEN_SECRET, (err, userData) => {
    if (userData) {
      ...
      return next(); 
      // you have the user_id now so you can proceed as normal
    };
    if (err) {
...
tokenController.deleteTokenFromDb = (req, res, next) => {
  ...
  // this will also tell us if the access token is in the DB to begin with
  db.query(tokens.deleteAccessToken, [access_token], (err, result) => {
    if (result) {
      if (result.affectedRows === 0) {
        // if we end up here, it means the token was ALREADY deleted. Therefore, a hacker got it
